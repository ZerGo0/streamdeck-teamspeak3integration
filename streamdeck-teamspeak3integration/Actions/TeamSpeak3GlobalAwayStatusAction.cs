...
{
    [PluginActionId("com.zergo0.teamspeak3integration.toggleglobalawaystatus")]
    public class TeamSpeak3AwayStatusAction : PluginBase
    {
        ...
        public override async void OnTick()
        {
            try
            {
                ...
                var awayStatus = TeamSpeak3Telnet.GetGlobalAwayStatus(clientId);
        ...
        private async Task ToggleAwayStatus(int desiredState = -1)
        {
            try
            {
                ...
                int awayStatus;
                if (desiredState == -1)
                    awayStatus = TeamSpeak3Telnet.GetGlobalAwayStatus(clientId);
                else
                ...
                switch (awayStatus)
                {
                    case -1:
                        return;
                    case 0:
                        setAwayStatus = TeamSpeak3Telnet.SetGlobalAwayStatus(1);
                        if (_settings.AwayStatusMessage.Length > 0)
                            TeamSpeak3Telnet.SetGlobalAwayMessage(_settings.AwayStatusMessage);
                        break;
                    case 1:
                        setAwayStatus = TeamSpeak3Telnet.SetGlobalAwayStatus(0);
                        break;
                }
